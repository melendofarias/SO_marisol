...
class List {
  public:
    ...
    void AppendPriority(Item item); 	// Pone un thread en la lista de acuerdo
										//a su prioridad
...
template <class Item>
void
List<Item>::AppendPriority(Item item)
{
    ListNode *element = new ListNode(item, item->priority);

    if (IsEmpty()) {		// list is empty
	first = element;
	last = element;
    } else {			// else acomodarlo segun prioridad
	...
	while(anterior->key >= element->key && anterior->next!=NULL){			//si tienen la misma prioridad
			anterior=anterior->next;			//lo va a colocar despues en la lista
		
	}
	element->next= anterior->next;
	if (anterior->next =NULL){
		last->next=element;			
		last=element;		}
	anterior->next = element;		
    }
}
